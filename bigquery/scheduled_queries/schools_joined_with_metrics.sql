...
SELECT
  ...
  trust.trust_name,
  trust.trust_size,
  #calculate the school 'tag' - a legacy categorisation of school governance still used in some RDD reporting
  CASE
    WHEN trust.trust_size > 1 THEN "MAT"
    WHEN LOWER(school.establishment_type_group) LIKE "%academ%"
  OR LOWER(school.establishment_type_group) LIKE "%free%" THEN "SAT"
  ELSE
  "Single school"
END
  AS tag
