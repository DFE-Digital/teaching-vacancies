class Jobseekers::JobApplicationMailer < Jobseekers::BaseMailer
  def application_rejected(job_application)
    ...
    @template = NOTIFY_JOBSEEKER_APPLICATION_REJECTED_TEMPLATE
    @to = @jobseeker.email

    view_mail(@template, to: @to, subject: I18n.t("jobseekers.job_application_mailer.application_rejected.subject"))
  end
