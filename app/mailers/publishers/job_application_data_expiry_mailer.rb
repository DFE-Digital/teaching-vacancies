class Publishers::JobApplicationDataExpiryMailer < Publishers::BaseMailer
  include DatesHelper

  helper_method :expiration_date

  def job_application_data_expiry
    @template = NOTIFY_PUBLISHER_JOB_APPLICATION_DATA_EXPIRY_TEMPLATE
    @publisher = params[:publisher]
    @vacancy =  params[:vacancy]
    @to = @publisher.email
    @subject = I18n.t("publishers.job_application_data_expiry_mailer.job_application_data_expiry.subject", job_title: @vacancy.job_title, expiration_date: format_date(expiration_date))

    view_mail(@template, to: @to, subject: @subject)
  end

  private

  def expiration_date
    (@vacancy.expires_at + 1.year).to_date
  end
end
