module Publishers
  class JobApplicationMailer < BaseMailer
    ...
    def applications_received(contact_email)
      @vacancies_submitted_yesterday = PublishedVacancy.distinct
                                                       .joins(:job_applications)
                                                       .where(contact_email: contact_email)
                                                       .merge(::JobApplication.submitted_yesterday)

      job_applications_count = @vacancies_submitted_yesterday.sum { |vacancy| vacancy.job_applications.submitted_yesterday.count }

      template = ERB.new(Rails.root.join("app/views/publishers/job_application_mailer/applications_received.text.erb").read)

      @vacancies = @vacancies_submitted_yesterday.index_with { |v| { location: vacancy_job_location(v), link: organisation_job_job_applications_url(v.id) } }
    ...
    def vacancies_job_applications
      @vacancies_submitted_yesterday.each_with_object({}) do |vacancy, hash|
