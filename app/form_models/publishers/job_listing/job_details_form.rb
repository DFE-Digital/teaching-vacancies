class Publishers::JobListing::JobDetailsForm < Publishers::JobListing::VacancyForm
  ...
  attr_accessor :job_title, :contract_type, :fixed_term_contract_duration, :maternity_cover_contract_duration, :key_stages, :subjects, :job_location, :readable_job_location, :status
  ...
  validates :maternity_cover_contract_duration, presence: true, if: -> { contract_type == "maternity_cover" }

  def self.fields
    %i[job_title contract_type fixed_term_contract_duration maternity_cover_contract_duration key_stages subjects]
  end
