class Jobseekers::SubscriptionForm
  ...
  def location_and_one_other_criterion_selected
    errors.add(:base, I18n.t("subscriptions.errors.no_location_or_other_criterion_selected")) if
      location.blank? || %i[keyword job_roles phases working_patterns].none? { |criterion| public_send(criterion).present? }
  end
