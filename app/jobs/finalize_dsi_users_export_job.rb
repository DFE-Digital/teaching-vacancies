...
class FinalizeDSIUsersExportJob < ApplicationJob
  queue_as :low
  ...
  def perform(run_id)
    ...
    unless run.all_pages_received?
      run.failed!
      raise "DSIExportRun #{run_id} (users) finalized without all pages present " \
            "(#{run.dsi_export_run_pages.count}/#{run.total_pages})"
    end

    pages = run.dsi_export_run_pages.order(:page_number).pluck(:payload)
    Publishers::DfeSignIn::BigQueryExport::Users.new.insert_pages(pages)
