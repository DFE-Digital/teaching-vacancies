class Jobseekers::Profiles::QualificationsController < Jobseekers::ProfilesController
  ...
  def qualification_params
    ...
    when "create", "edit", "update"
      params.expect(qualification_form_param_key(@category) => [:category, :finished_studying, :finished_studying_details,
                                                                :grade, :institution, :name, :subject, :year,
                                                                :awarding_body, { qualification_results_attributes: [%i[id subject grade awarding_body]] }])
    end
  end
