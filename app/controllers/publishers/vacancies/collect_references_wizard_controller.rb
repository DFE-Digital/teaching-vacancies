...
module Publishers
  module Vacancies
    class CollectReferencesWizardController < ReferencesAndSelfDisclosureWizardController
      ...
      def show
        if step != Wicked::FINISH_STEP
          # we only display the second step if the first one is 'yes' so no need for anything
          # more clever here
          @form = form_class.new
        end
        render_wizard
