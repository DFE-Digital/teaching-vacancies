class Publishers::Vacancies::JobApplicationsController < Publishers::Vacancies::BaseController
  ...
  def status
    return "shortlisted" if params[:publishers_job_application_update_status_form].key?("further_instructions")

    "unsuccessful" if params[:publishers_job_application_update_status_form].key?("rejection_reasons")
  end
