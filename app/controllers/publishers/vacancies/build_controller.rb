class Publishers::Vacancies::BuildController < Publishers::Vacancies::BaseController
  ...
  steps :job_roles, :job_roles_more, :job_location, :schools, :job_details, :pay_package, :important_dates, :documents,
        :applying_for_the_job, :job_summary

  helper_method :back_path, :form, :job_roles, :job_roles_more_options
  ...
  def show
    case step
    when :job_roles_more
      skip_step if job_roles.include?("sendco")
    when :job_location
  ...
  def back_path
    @back_path ||= if session[:current_step] == :review
                     finish_wizard_path
                   elsif previous_step == :schools && !current_organisation.school? && vacancy.central_office?
                     wizard_path(:job_location)
                   elsif previous_step == :job_roles_more && vacancy.sendco?
                     wizard_path(:job_roles)
                   else
  ...
  def job_roles
    @job_roles ||= session[:job_roles].presence || vacancy.job_roles
  end
  ...
  def next_part_of_step_required?
    (step == :job_location && job_location != "central_office") ||
      (step == :job_roles && job_roles.exclude?("sendco"))
  end
