module Publishers
  class Vacancies::BatchEmailsController < Vacancies::BaseController
    ...
    def send_rejection_emails
      ...
      cc = [form.contact_email] if form.email_copy
      logo_org = vacancy.organisation if form.include_school_logo

      batch.job_applications.each do |job_application|
        BatchRejectionMailer.send_rejection(from: form.from, logo_org: logo_org, cc: cc, subject: form.subject, content: form.content, job_application: job_application).deliver_later
      end
