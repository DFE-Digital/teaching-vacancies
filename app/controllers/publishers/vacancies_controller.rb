class Publishers::VacanciesController < Publishers::Vacancies::BaseController
  ...
  def redirect_to_new_features_reminder
    return redirect_to reminder_new_features_path if show_application_reminder_page? && current_publisher.viewed_application_feature_reminder_page_at.nil?
  end

  def show_application_reminder_page?
    vacancies_published = Vacancy.published.where("created_at > ? ", current_publisher.viewed_new_features_page_at)
    current_publisher.viewed_new_features_page_at.present? && vacancies_published&.any? && vacancies_published&.any?(&:enable_job_applications?).blank?
  end
