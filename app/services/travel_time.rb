...
class TravelTime
  ...
  def commute_area
    Rails.cache.fetch(cache_key, expires_in: 1.month) { travel_time_commute_area }
  end

  private

  def travel_time_commute_area
  ...
  def cache_key
    [@location&.downcase, @transportation_type, @travel_time_in_minutes]
  end
end
