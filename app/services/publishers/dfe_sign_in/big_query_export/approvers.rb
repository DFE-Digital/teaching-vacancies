module Publishers::DfeSignIn::BigQueryExport
  class Approvers < Base
    ...
    def call
      # Insert page by page rather than loading every approver into memory at once. The
      # table is only deleted once the first page has been fetched successfully, so a DSI
      # failure on page 1 leaves it untouched; a failure on a later page leaves the table
      # holding whichever earlier pages already made it in, rather than nothing at all.
      (1..dsi_approvers_page_count).each do |page_number|
        page = dsi_approvers_page(page_number)
