...
class Search::Postgres::QueryParser < Parslet::Parser
  ...
  # Convenience method to parse and transform in one fell swoop
  def self.arel_from_query(query)
    parsed_tree = new.parse(query)
    Search::Postgres::QueryTransformer.new.apply(parsed_tree)
  end

  # Represents any phrase that occurs in the synonym dictionary as a static string token
  def synonym_token
    Rails.application.config.x.search.synonym_triggers
      .map { |s| str(s) }.reduce(:|)
  end

  # Represents any phrase that occurs in the oneway synonym dictionary as a static string token
  def oneway_synonym_token
    Rails.application.config.x.search.oneway_synonym_triggers
      .map { |s| str(s) }.reduce(:|)
  end
