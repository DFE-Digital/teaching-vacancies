...
import {
  connectSearchBox, connectAutocomplete, connectHits, connectSortBy, connectMenu,
...
import {
  updateUrlQueryParams, removeDataAttribute, setDataAttribute,
} from '../lib/utils';
import { getGeolocatedCoordinates } from '../lib/api';
import { enableSubmitButton } from './ui/form';
import { shouldGeocodeQuery } from './query';
...
if (document.querySelector('#vacancies-hits')) {
  ...
  const locationSearchBox = searchBox({
    ...
    onChange: (query) => new Promise((resolve) => {
      if (shouldGeocodeQuery(query, locations)) {
