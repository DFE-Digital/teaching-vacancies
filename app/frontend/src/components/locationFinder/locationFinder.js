...
const containerEl = document.getElementsByClassName('location-finder')[0];
const inputEl = document.getElementsByClassName('location-finder__input')[0];
...
export const startLoading = (container, input) => {
  input.disabled = true;
  container.classList.add('location-finder--loading');
...
export const stopLoading = (container, input) => {
  ...
  container.classList.remove('location-finder--loading');
...
export const showErrorMessage = (link) => {
  if (!document.querySelector('.location-finder__link .govuk-error-message')) {
    const errorMessage = document.createElement('div');
    errorMessage.setAttribute('role', 'alert');
    errorMessage.id = 'location-finder__error';
...
export const removeErrorMessage = () => {
  if (document.querySelector('.location-finder__link .govuk-error-message')) {
    document.querySelector('.location-finder__link .govuk-error-message').remove();
  }
};
