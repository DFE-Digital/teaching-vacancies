...
export default class extends Controller {
  ...
  marker({ point, meta }) {
    ...
    L.marker(point, { icon }).addTo(this.map).bindPopup(`<h4 class="govuk-body-m"><a href="${meta.name_link}">${meta.name}</a></h4><p>${meta.school_type}</p><p>${meta.address}</p>`).openPopup();
  }
}
