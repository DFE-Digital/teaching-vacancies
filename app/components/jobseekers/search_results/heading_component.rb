  ...
  def heading
    if @keyword.present? && @polygon_boundaries.present?
      I18n.t("jobs.search_result_heading.keyword_location_polygon_html",
             jobs_count: number_with_delimiter(@total_count), location: @location, keyword: @keyword, count: @total_count, radius: "#{@radius} #{I18n.t("jobs.search_result_heading.unit_of_length").pluralize(@radius.to_i)}")
      ...
      I18n.t("jobs.search_result_heading.location_polygon_html",
             jobs_count: number_with_delimiter(@total_count), location: @location, count: @total_count, radius: "#{@radius} #{I18n.t("jobs.search_result_heading.unit_of_length").pluralize(@radius.to_i)}")
