class SubscriptionPresenter < BasePresenter
  ...
  def search_criteria_field(field, value)
    ...
    when "radius", "sort_by"
      return
    when "location"
      return render_location_filter(value, search_criteria["radius"])
    when "job_roles"
      return render_job_roles_filter(value)
    when "working_patterns"
      return render_working_patterns_filter(value)
    when "phases"
      return render_phases_filter(value)
    when "newly_qualified_teacher"
      return render_ect_filter(value)
    else
      { "#{field}": value }
    end
  end
