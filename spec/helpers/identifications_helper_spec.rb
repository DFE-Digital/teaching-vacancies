...
RSpec.describe IdentificationsHelper, type: :helper do
  describe '#identification_options' do
    before(:each) do
      ...
      stub_const("IdentificationsHelper::DFE_SIGN_IN_OPTIONS", [OpenStruct.new(name: 'test_dfe_sign_in_region', to_radio: ['test_dfe_sign_in_region', 'Test DfE Sign In region'])])
      stub_const("IdentificationsHelper::AZURE_SIGN_IN_OPTIONS", [OpenStruct.new(name: 'test_azure_region', to_radio: ['test_azure_region', 'Test azure region'])])
    end
