...
RSpec.describe Jobseekers::ProfessionalBodyMembershipForm, type: :model do
  ...
  let(:date_membership_obtained) { Date.today }
  let(:exam_taken) { true }
  ...
  describe "validations" do
    ...
    context "when name is not present" do
      ...
      it "is not valid" do
        ...
        expect(subject.errors[:name]).to include("Enter the name of the professional body.")
      end
    end
    ...
    context "when exam_taken is not a valid value" do
      ...
      it "is not valid" do
        expect(subject).not_to be_valid
        expect(subject.errors[:exam_taken]).to include("Select yes if your professional body membership required you to take an exam.")
      end
    end

    context "when exam_taken is true or false" do
      it "is valid with true" do
        subject.exam_taken = true
        expect(subject).to be_valid
      end

      it "is valid with false" do
        subject.exam_taken = false
        expect(subject).to be_valid
      end
    end
