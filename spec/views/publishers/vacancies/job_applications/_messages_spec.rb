...
RSpec.describe "publishers/vacancies/job_applications/_messages.html.slim" do
  ...
  before do
    ...
    assign(:message_form, double("MessageForm")) # rubocop:disable RSpec/VerifiedDoubles
  ...
  context "when messaging is allowed" do
    ...
    it "shows send message button and hides disabled messages button button" do
      render partial: "publishers/vacancies/job_applications/messages",
             locals: { messages: messages, job_application: job_application, vacancy: vacancy }

      expect(rendered).to have_link("Print this page")
      expect(rendered).to have_link("Send message to candidate")
