      ...
      it "shows all vacancy information" do
        vacancy.organisations.each do |organisation|
          if organisation.school?
            ...
            expect(rendered).to have_content("#{I18n.t('organisations.job_location_heading.central_office')}, #{full_address(organisation)}")
          end
        end

        expect(rendered).to have_content(vacancy_presenter.readable_job_roles)
        expect(rendered).to have_content(vacancy.job_title)
        expect(rendered).to have_content(vacancy_presenter.readable_key_stages) if vacancy.key_stages.present?
        expect(rendered).to have_content(vacancy_presenter.readable_subjects) if vacancy.subjects.any?
