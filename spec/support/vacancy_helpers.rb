module VacancyHelpers
  ...
  def fill_in_applying_for_the_job_form_fields(vacancy, local_authority_vacancy: false)
    if JobseekerApplicationsFeature.enabled? && !local_authority_vacancy && vacancy.enable_job_applications?
      choose strip_tags(I18n.t("helpers.label.publishers_job_listing_applying_for_the_job_form.enable_job_applications_options.true_html"))
  ...
  def verify_all_vacancy_details(vacancy)
    ...
    expect(page).to have_content(vacancy.expires_at.to_s.strip)
  ...
  def verify_shared_vacancy_list_page_details(vacancy)
    ...
    expect(page.find(".vacancy")).to have_content(vacancy.expires_at)
