...
RSpec.describe VacancyDecorator do
  ...
  describe "#readable_working_patterns" do
    context "when is_job_share" do
      ...
      it "returns working patterns" do
        expect(decorated.readable_working_patterns).to eq("Full time, part time (Can be done as a job share)")
      end
    end
    ...
    context "when is_job_share == false" do
      ...
      it "returns working patterns" do
        expect(decorated.readable_working_patterns).to eq("Full time, part time")
      end
    end
  end
  ...
  describe "#readable_working_patterns_with_details" do
    ...
    it "returns the working with details" do
      expect(decorated.readable_working_patterns_with_details).to eq("Full time, part time: Some details")
    end
    ...
    context "when there is no details" do
      ...
      it "returns the working patterns" do
        expect(decorated.readable_working_patterns_with_details).to eq("Full time, part time")
      end
    end
  end
  ...
  describe "#fixed_term_contract_duration" do
    ...
    context "when permanent" do
      ...
      it "returns Permanent" do
        expect(decorated.contract_type_with_duration).to eq "Permanent"
      end
    end
    ...
    context "when fixed term" do
      ...
      context "when is_parental_leave_cover is false" do
        ...
        it "returns Fixed term (duration)" do
          expect(decorated.contract_type_with_duration).to eq "Fixed term - 6 months"
        end
      end
      ...
      context "when is_parental_leave_cover is true" do
        ...
        it "returns Fixed term (duration)" do
          expect(decorated.contract_type_with_duration).to eq "Fixed term - 6 months - Maternity or parental leave cover"
        end
      end
    end
  end
  ...
  describe "#readable_subjects" do
    ...
    it "joins them correctly" do
      expect(decorated.readable_subjects).to eq("Acrobatics, Tapestry")
    end
    ...
    context "when there are no subjects" do
      ...
      it "returns empty string" do
        expect(decorated.readable_subjects).to be_blank
      end
    end
  end
