...
RSpec.describe ExportDSIApproversToBigQueryJob do
  context "when DisableIntegrations is not enabled" do
    it "creates a run and fans out one FetchDSIApproversExportPageJob per page" do
      fetch_dsi_users = instance_double(Publishers::DfeSignIn::FetchDSIUsers, dsi_approvers_page_count: 3)
      ...
      expect { described_class.perform_now }
        ...
        .and have_enqueued_job(FetchDSIApproversExportPageJob).exactly(3).times

      run = DSIExportRun.last
      expect(run.source).to eq("approvers")
      expect(run.total_pages).to eq(3)
      expect(run).to be_running
    end

    it "does not start a new run while an approvers export is already running" do
      DSIExportRun.create!(source: "approvers", total_pages: 5)
