...
RSpec.describe Vacancy do
  ...
  describe "#set_mean_geolocation_from_postcode!" do
    let(:school) { create(:school, postcode: "A12 B34") }
    ...
    context "when in a trust" do
      ...
      context "when at multiple schools in a school group" do
        ...
        before do
          allow(geocoding).to receive(:postcode_from_coordinates).and_return(Geocoder::DEFAULT_LOCATION)

          # This column is normally autogenerated based on easting and northing, so we need to
          # manually force it to a simple value for the sake of this test
          school.update_column(:geolocation, [1, 2])
          school2.update_column(:geolocation, [5, 6])
        end
