...
RSpec.describe Vacancy do
  ...
  context "scopes" do
    ...
    describe "#expires_within_data_retention_period" do
      ...
      it "retrieves vacancies that expired not more than one year ago" do
        expect(Vacancy.expires_within_data_retention_period).to_not include(expired_years_ago)
        expect(Vacancy.expires_within_data_retention_period).to include(expired_earlier_today)
      end
    end
