...
RSpec.describe Jobseekers::SearchResults::HeadingComponent, type: :component do
  ...
  context "when keyword and search polygon boundaries are present" do
    it "renders correct heading" do
      expect(rendered_component).to include(
        I18n.t("jobs.search_result_heading.keyword_location_polygon_html", keyword: keyword, location: location, count: count, radius: "#{radius} #{I18n.t('jobs.search_result_heading.unit_of_length').pluralize(radius.to_i)}"),
      )
    end
    ...
    context "and a buffer radius is applied" do
      ...
      it "renders correct heading" do
        expect(rendered_component).to include(
          I18n.t("jobs.search_result_heading.keyword_location_html", keyword: keyword, location: location, count: count, radius: "#{radius} #{I18n.t('jobs.search_result_heading.unit_of_length').pluralize(radius.to_i)}"),
        )
      end
    end
  end
  ...
  context "when search polygon boundaries is present but keyword is not present" do
    ...
    it "renders correct heading" do
      expect(rendered_component).to include(
        I18n.t("jobs.search_result_heading.location_polygon_html", location: location, count: count, radius: "#{radius} #{I18n.t('jobs.search_result_heading.unit_of_length').pluralize(radius.to_i)}"),
      )
    end
    ...
    context "and a buffer radius is applied" do
      ...
      it "renders correct heading" do
        expect(rendered_component).to include(
          I18n.t("jobs.search_result_heading.location_html", location: location, count: count, radius: "#{radius} #{I18n.t('jobs.search_result_heading.unit_of_length').pluralize(radius.to_i)}"),
        )
      end
    end
  end
