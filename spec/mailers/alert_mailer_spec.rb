...
RSpec.describe AlertMailer, type: :mailer do
  ...
  context 'when frequency is daily' do
    ...
    it 'sends a job alert email' do
      ...
      expect(body).to include(vacancies.first.job_title)
      expect(body).to include(vacancies.first.share_url(**campaign_params))
      ...
      expect(body).to include(subscription_presenter.edit_url(**campaign_params).gsub('&', '&amp;'))
      ...
      expect(body).to include(subscription_presenter.unsubscribe_url(**campaign_params).gsub('&', '&amp;'))
    end
  end
