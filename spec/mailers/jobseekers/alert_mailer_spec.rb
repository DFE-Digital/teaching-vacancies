...
RSpec.describe Jobseekers::AlertMailer, type: :mailer do
  ...
  context "when frequency is daily" do
    ...
    it "sends a job alert email" do
      ...
      expect(body).to include(I18n.t("jobseekers.alert_mailer.alert.salary", salary: vacancies.first.salary))
      expect(body).to include(I18n.t("jobseekers.alert_mailer.alert.working_pattern", working_pattern: vacancies.first.working_patterns))
      expect(body).to include(I18n.t("jobseekers.alert_mailer.alert.closing_date", closing_date: format_date(vacancies.first.expires_on)))
      expect(body).to include(I18n.t("jobseekers.alert_mailer.alert.title"))
      expect(body).to include(I18n.t("subscriptions.intro"))
      expect(body).to include("Keyword: English")
      expect(body).to include(I18n.t("jobseekers.alert_mailer.alert.alert_frequency", frequency: subscription.frequency))
      expect(body).to include(I18n.t("jobseekers.alert_mailer.alert.edit_link_text"))
      expect(body).to include(edit_subscription_url(subscription.token, **campaign_params))
      expect(body).to include(I18n.t("jobseekers.alert_mailer.alert.feedback.heading"))
      expect(body).to match(/(\[#{I18n.t('jobseekers.alert_mailer.alert.feedback.relevant_link_text')}\]\(.+true)/)
      expect(body).to include(relevant_job_alert_feedback_url)
      expect(body).to match(/(\[#{I18n.t('jobseekers.alert_mailer.alert.feedback.irrelevant_link_text')}\]\(.+false)/)
      expect(body).to include(irrelevant_job_alert_feedback_url)
      expect(body).to include(I18n.t("jobseekers.alert_mailer.alert.feedback.reason"))
