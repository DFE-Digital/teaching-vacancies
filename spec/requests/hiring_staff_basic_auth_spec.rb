...
RSpec.shared_examples 'hiring staff basic auth' do |path|
  ...
  it 'asks for the basic auth credentials' do
    ...
    allow(fake_env).to receive(:hiring_staff_http_user?).and_return(true)
    allow(fake_env).to receive(:hiring_staff_http_pass?).and_return(true)
    allow(fake_env).to receive(:hiring_staff_http_user).and_return('username')
    allow(fake_env).to receive(:hiring_staff_http_pass).and_return('password')
  ...
  context 'when the incorrect basic auth credentials are given' do
    it 'returns a 401' do
      fake_env = double.as_null_object
      allow(Figaro).to receive(:env).and_return(fake_env)
      allow(fake_env).to receive(:hiring_staff_http_user?).and_return(false)
      allow(fake_env).to receive(:hiring_staff_http_pass?).and_return(false)
      allow(fake_env).to receive(:hiring_staff_http_user).and_return(nil)
      allow(fake_env).to receive(:hiring_staff_http_pass).and_return(nil)
...
RSpec.shared_examples 'does not require basic auth for hiring staff' do |path|
  let(:path) { path.present? ? path : '/' }
...
RSpec.describe 'hiring staff basic auth', type: :request do
  context 'when in development' do
    it_behaves_like 'does not require basic auth for hiring staff', '/schools'
  end
  ...
  context 'when in staging' do
    ...
    it_behaves_like 'hiring staff basic auth', '/schools'
  end
