...
RSpec.describe "Publishers can manage settings" do
  ...
  context "when publisher logs in as a school" do
    ...
    it "allows to edit the school details" do
      ...
      expect(find_field("publishers_organisation_form[website]").value).to eq(organisation.url)
  ...
  context "when publisher logs in as a trust" do
    ...
    let(:school1) { create(:school, website: "http://example.com") }
    let(:school2) { create(:school) }

    it "allows to edit the trust details" do
      click_link("Change", match: :first)

      expect(find_field("publishers_organisation_form[website]").value).to eq(organisation.website)
    ...
    it "allows to edit details of a school in the trust" do
      ...
      expect(find_field("publishers_organisation_form[website]").value).to eq(school1.website)
