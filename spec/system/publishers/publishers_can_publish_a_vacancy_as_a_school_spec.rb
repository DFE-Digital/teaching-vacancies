...
RSpec.describe "Creating a vacancy" do
  ...
  context "with a non-faith school" do
    ...
    context "with an external application form and a school that triggers the key stages form" do
      ...
      it "follows the non-TVS flow" do
        ...
        expect(page).to have_content("Tell jobseekers about your learners and the culture of your school", count: 1)
        expect(page).to have_content(I18n.t("jobs.school_offer.hint"), count: 1)
        submit_empty_form
        expect(publisher_about_the_role_page).to be_displayed
  ...
  def fill_in_forms_until_applying_for(vacancy, created_vacancy_id)
    ...
    fill_in_important_dates_form_fields(publish_on: vacancy.publish_on, expires_at: vacancy.expires_at)
    click_on I18n.t("buttons.save_and_continue")
