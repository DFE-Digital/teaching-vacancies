...
RSpec.describe "Publishers can view a vacancy's activity log", :versioning do
  ...
  it "updates the activity log" do
    ...
    expect(page).to have_current_path(organisation_job_build_path(vacancy.id, :subjects), ignore_query: true)
    ...
    expect(page).to have_current_path(organisation_job_build_path(vacancy.id, :contract_type), ignore_query: true)
