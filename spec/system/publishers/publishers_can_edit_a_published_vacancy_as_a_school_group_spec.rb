...
RSpec.describe "Editing a published vacancy" do
  ...
  let(:vacancy) { create(:vacancy, organisations: [school_group], phases: %w[primary], key_stages: %w[ks1]) }
  ...
  describe "#job_location" do
    scenario "can edit job location" do
      ...
      expect(vacancy.reload.organisations).to contain_exactly(school1)

      change_job_locations(vacancy, [school2])
      click_on I18n.t("buttons.save_and_continue")

      expect(page.current_path).to eq(organisation_job_path(vacancy.id))
      expect(vacancy.reload.organisations).to contain_exactly(school2)

      change_job_locations(vacancy, [school1, school2])
      click_on I18n.t("buttons.save_and_continue")

      expect(page.current_path).to eq(organisation_job_path(vacancy.id))
      expect(vacancy.reload.organisations).to contain_exactly(school1, school2)
      ...
      expect(page.current_path).to eq(organisation_job_path(vacancy.id))
      expect(vacancy.reload.organisations).to contain_exactly(school_group)
    end
  end
end
