...
RSpec.describe "Publishers can view a job application" do
  ...
  context "when job application is an uploaded job application" do
    it "allows hiring staff to see jobseeker personal details" do
      ...
      expect(page).to have_content "Personal details"
      expect(page).to have_css(".govuk-summary-list__key", text: "First name")
      expect(page).to have_css(".govuk-summary-list__value", text: uploaded_job_application.first_name)
  
      expect(page).to have_css(".govuk-summary-list__key", text: "Last name")
      expect(page).to have_css(".govuk-summary-list__value", text: uploaded_job_application.last_name)
  
      expect(page).to have_css(".govuk-summary-list__key", text: "Phone number")
      expect(page).to have_css(".govuk-summary-list__value", text: uploaded_job_application.phone_number)
  
      expect(page).to have_css(".govuk-summary-list__key", text: "Email address")
      expect(page).to have_css(".govuk-summary-list__value", text: uploaded_job_application.email)
  
      expect(page).to have_css(".govuk-summary-list__key", text: "Do you need Skilled Worker visa sponsorship?")
      expect(page).to have_css(".govuk-summary-list__value", text: I18n.t("jobseekers.profiles.personal_details.work.options.true"))

      expect(page).to have_css(".govuk-summary-list__key", text: "Teacher reference number (TRN)")
      expect(page).to have_css(".govuk-summary-list__value", text: uploaded_job_application.teacher_reference_number)


      expect(page).to have_link("Download application")

      link = find_link("Download application")[:href]

      expect(link).to eq(
        organisation_job_job_application_download_application_form_path(
          uploaded_job_application.vacancy.id,
          uploaded_job_application
        )
      )
    end
  end
