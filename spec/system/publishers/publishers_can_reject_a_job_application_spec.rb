...
RSpec.describe "Publishers can reject a job application" do
  ...
  describe "rejecting applications" do
    ...
    describe "whole rejection flow", :js do
      ...
      let(:batch_email) { BatchEmail.order(:created_at).last }
    ...
    describe "template flow" do
      let(:batch_email) { create(:batch_email, vacancy: vacancy, job_applications: JobApplication.all) }
      ...
      describe "rejecting applications", :js do
        ...
        it "handles the rejection email process" do
          ...
          #  wait for page to load
          find_by_id("tab_not_considering")
        ...
        scenario "copying email to originator" do
          ...
          perform_enqueued_jobs
          expect(ActionMailer::Base.deliveries.map(&:cc).uniq).to eq([[publisher.email]])
        end
