...
RSpec.describe "Jobseekers can change email" do
  ...
  describe "updating email" do
    it "validates and submits the form, sends confirmation and notification emails and redirects to check your email page" do
      ...
      expect(delivered_emails.second.subject).to eq(I18n.t("jobseeker_mailer.confirmation_instructions.subject"))
      expect(delivered_emails.second.to.first).to eq("new@email.com")
      expect(current_path).to eql(jobseekers_check_your_email_path)
    end
  end

  describe "confirming email" do
    it "confirms the email, updates the email, and redirects to saved_jobs page" do
      update_jobseeker_email("new@email.com", jobseeker.password)
