  ...
  context "when a vacancy has expired and it has applications" do
    ...
    describe "the summary section" do
      ...
      it "shows vacancy title and deadline passed" do
        ...
        within(".vacancy-deadline") do
          ...
          expect(page).to have_content(I18n.t("publishers.vacancies.job_applications.deadline.after"))
        end
      end
      ...
      it "shows total of each status for all applications" do
        within(".application-count.govuk-tag--green") do
          ...
          expect(page).to have_content(I18n.t("publishers.vacancies.job_applications.shortlisted"))
        end

        within(".application-count.govuk-tag--blue") do
          expect(page).to have_content("1")
          expect(page).to have_content(I18n.t("publishers.vacancies.job_applications.review"))
        end

        within(".application-count.govuk-tag--red") do
          expect(page).to have_content("1")
          expect(page).to have_content(I18n.t("publishers.vacancies.job_applications.rejected"))
        end
      end
      ...
      it "shows correct vacancy actions available to the publisher" do
        within(".vacancy-actions") do
          expect(page).to have_css(".govuk-button", count: 3)
          expect(page).to have_link(I18n.t("publishers.vacancies.job_applications.buttons.relist"), href: "/")
    ...
    describe "submitted application" do
      ...
      it "has action to reject application" do
        within(".application-#{status} .card-component__actions") do
          expect(page).to have_link(I18n.t("publishers.vacancies.job_applications.actions.reject"), href: organisation_job_job_application_reject_path(vacancy.id, job_application_submitted.id))
        end
      end

      it "has action to shortlist application" do
        within(".application-#{status} .card-component__actions") do
          expect(page).to have_link(I18n.t("publishers.vacancies.job_applications.actions.shortlist"), href: organisation_job_job_application_shortlist_path(vacancy.id, job_application_submitted.id))
        end
      end
    end
    ...
    describe "shortlisted application" do
      ...
      it "has action to reject application only" do
        within(".application-#{status} .card-component__actions") do
          expect(page).to have_link(I18n.t("publishers.vacancies.job_applications.actions.reject"), href: organisation_job_job_application_reject_path(vacancy.id, job_application_shortlisted.id))
          expect(page).not_to have_link(I18n.t("publishers.vacancies.job_applications.actions.shortlist"), href: organisation_job_job_application_shortlist_path(vacancy.id, job_application_shortlisted.id))
        end
      end
    end
    ...
    describe "unsuccessful application" do
      ...
      it "has no actions" do
        within(".application-#{status} .card-component__actions") do
          expect(page).not_to have_link(I18n.t("publishers.vacancies.job_applications.actions.reject"), href: organisation_job_job_application_reject_path(vacancy.id, job_application_unsuccessful.id))
          expect(page).not_to have_link(I18n.t("publishers.vacancies.job_applications.actions.shortlist"), href: organisation_job_job_application_shortlist_path(vacancy.id, job_application_unsuccessful.id))
        end
      end
    end
  end
  ...
  context "when a vacancy is active and it has no applications" do
    ...
    describe "the summary section" do
      ...
      it "shows vacancy title and deadline date" do
        ...
        within(".vacancy-deadline") do
          expect(page).to have_content(expiry_date_and_time(vacancy))
          expect(page).to have_content(I18n.t("publishers.vacancies.job_applications.deadline.before"))
        end
      end
      ...
      it "shows that there are no applicants" do
        expect(page).to have_css(".empty-section-component h3", text: I18n.t("publishers.vacancies.job_applications.no_applicants"))
      end
