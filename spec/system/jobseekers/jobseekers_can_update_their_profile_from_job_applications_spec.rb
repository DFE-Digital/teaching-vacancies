...
RSpec.describe "Jobseekers can update their profile from job applications" do
  ...
  context "when the jobseekers have a profile" do
    ...
    let!(:jobseeker_profile) { create(:jobseeker_profile, jobseeker: jobseeker, qualifications: [profile_qualification], employments: [profile_employment], training_and_cpds: [profile_training]) }
    ...
    it "jobseekers can update their profile qualifications using the job application information on submission" do
      ...
      expect(page).to have_no_css("h3.govuk-summary-card__title", text: "Application employment")

      expect(page).to have_css("h3.govuk-summary-card__title", text: "Original profile training")
      expect(page).to have_no_css("h3.govuk-summary-card__title", text: "Application training")
    end

    it "jobseekers can update their profile work history using the job application information on submission" do
      ...
      expect(page).to have_no_css("h3.govuk-summary-card__title", text: "Original profile employment")

      expect(page).to have_css("h3.govuk-summary-card__title", text: "Original profile training")
      expect(page).to have_no_css("h3.govuk-summary-card__title", text: "Application training")
    end

    it "jobseekers can update their profile training and cpd using the job application information on submission" do
      ...
      expect(page).to have_no_css("h3.govuk-summary-card__title", text: "Original profile training")
  ...
  context "when the jobseekers do not have a profile" do
    ...
    it "they are not offered to update their qualifications or work history on submission" do
      login_as(jobseeker, scope: :jobseeker)
      visit jobseekers_job_application_review_path(job_application)
      expect(page.body).to have_no_content(I18n.t(".jobseekers.job_applications.review.confirmation.update_your_profile.heading"))
    end
  end
end
