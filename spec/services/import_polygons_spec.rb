...
RSpec.describe ImportPolygons do
  ...
  describe "#call" do
    ...
    describe "#get_buffers" do
      ...
      context "when the buffer radii are different from the last time the task was run" do
        let(:new_buffers) do
          {
            "1" => [buffer_polygon],
            "5" => [buffer_polygon],
        ...
        before do
          stub_const("ImportPolygons::BUFFER_DISTANCES_IN_MILES", [1, 5, 10, 25, 50, 100, 200, 300])
          ImportPolygons::BUFFER_DISTANCES_IN_MILES.each do |distance|
      ...
      context "when the buffer radii are the same as the last time the task was run" do
