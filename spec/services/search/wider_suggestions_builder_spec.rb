...
RSpec.describe Search::WiderSuggestionsBuilder do
  ...
  describe ".call" do
    ...
    context "returns suggestions" do
      ...
      before do
        allow(initial_search.class).to receive(:new).with(hash_including(radius: 10)).and_return(pg_search)
        allow(initial_search.class).to receive(:new).with(hash_including(radius: 15)).and_return(pg_search)
        allow(initial_search.class).to receive(:new).with(hash_including(radius: 20)).and_return(pg_search)
        allow(initial_search.class).to receive(:new).with(hash_including(radius: 25)).and_return(pg_search)
        allow(initial_search.class).to receive(:new).with(hash_including(radius: 50)).and_return(pg_search)
        allow(initial_search.class).to receive(:new).with(hash_including(radius: 100)).and_return(pg_search)
        allow(initial_search.class).to receive(:new).with(hash_including(radius: 200)).and_return(pg_search)
      end
