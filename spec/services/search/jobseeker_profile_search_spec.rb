...
RSpec.describe Search::JobseekerProfileSearch, :geocode do
  ...
  context "when the results are only filtered by organisation" do
    ...
    context "when the organisation is a trust", :vcr do
      ...
      before do
        create(:job_preferences_location, name: "N7 7DD", radius: 1, job_preferences:)
        create(:job_preferences_location, name: "SE1 9NA", radius: 1, job_preferences: job_preferences2)
  ...
  context "when using other jobseeker profile filters" do
    ...
    describe "jobseeker_profile qualified_teacher_status", :vcr do
      ...
      before do
        create(:job_preferences_location, **location_preference, job_preferences: qts_job_preferences)
        create(:job_preferences_location, **location_preference, job_preferences: no_qts_job_preferences)
        create(:job_preferences_location, **location_preference, job_preferences: nil_qts_job_preferences)
        create(:job_preferences_location, **location_preference, job_preferences: non_teacher_qts_job_preferences)
        create(:job_preferences_location, **location_preference, job_preferences: on_track_qts_job_preferences)
      end

      it "returns the jobseeker profiles with the qualified_teacher_status specified in the filters" do
      ...
      context "when hiring staff selects 'Does not have QTS'" do
        ...
        it "returns jobseekers with who answered 'No', 'I'm not looking for a teaching role' or didn't answer the QTS question" do
          expect(search.jobseeker_profiles.map(&:qualified_teacher_status)).to contain_exactly(no_qts_jobseeker_profile.qualified_teacher_status, nil_qts_jobseeker_profile.qualified_teacher_status, non_teacher_qts_jobseeker_profile.qualified_teacher_status)
        end
      end

      context "when searching using multiple QTS statuses" do
        ...
        it "returns the jobseeker profiles with the qualified_teacher_status specified in the filters" do
          expect(search.jobseeker_profiles).to contain_exactly(qts_jobseeker_profile, on_track_qts_jobseeker_profile)
        end
      end
    end
    ...
    describe "job_preferences roles", :vcr do
      ...
      it "onlies return the jobseeker profiles with the roles specified in the filters" do
        expect(search.jobseeker_profiles).to eq([cleaning_staff_jobseeker_profile])
      end

      context "when searching using multiple roles" do
        ...
        before { create(:job_preferences_location, **location_preference, job_preferences: headteacher_job_preferences) }

        it "returns the jobseeker profiles with the roles specified in the filters" do
          expect(search.jobseeker_profiles).to contain_exactly(teacher_jobseeker_profile, headteacher_jobseeker_profile)
        end
      end
    end

    describe "job_preferences working_patterns", :vcr do
      ...
      it "onlies return the jobseeker profiles with the working patterns specified in the filters" do
        expect(search.jobseeker_profiles).to eq([full_time_jobseeker_profile])
      end

      context "when searching using multiple working_patterns" do
        ...
        it "returns the jobseeker profiles with the working patterns specified in the filters" do
          expect(search.jobseeker_profiles).to contain_exactly(full_time_jobseeker_profile, part_time_jobseeker_profile)
        end
      end
    end

    describe "job_preferences phases", :vcr do
      ...
      before { create(:job_preferences_location, **location_preference, job_preferences: secondary_job_preferences) }

      it "onlies return the jobseeker profiles with the phases specified in the filters" do
        expect(search.jobseeker_profiles).to eq([secondary_jobseeker_profile])
      end

      context "when searching using multiple phases" do
        ...
        before { create(:job_preferences_location, **location_preference, job_preferences: primary_job_preferences) }

        it "returns the jobseeker profiles with the phases specified in the filters" do
          expect(search.jobseeker_profiles).to contain_exactly(secondary_jobseeker_profile, primary_jobseeker_profile)
        end
      end
    end

    describe "job_preferences key_stages", :vcr do
      ...
      before { create(:job_preferences_location, **location_preference, job_preferences: ks1_job_preferences) }

      it "onlies return the jobseeker profiles with the key_stages specified in the filters" do
        expect(search.jobseeker_profiles).to eq([ks1_jobseeker_profile])
      end

      context "when searching using multiple key_stages" do
        ...
        before { create(:job_preferences_location, **location_preference, job_preferences: ks2_job_preferences) }

        it "returns the jobseeker profiles with the key_stages specified in the filters" do
          expect(search.jobseeker_profiles).to contain_exactly(ks1_jobseeker_profile, ks2_jobseeker_profile)
        end
      end
    end

    describe "job_preferences subjects", :vcr do
      ...
      before { create(:job_preferences_location, **location_preference, job_preferences: history_job_preferences) }

      it "onlies return the jobseeker profiles with subjects present in the filters" do
        expect(search.jobseeker_profiles).to eq([history_jobseeker_profile])
      end

      context "when searching using multiple subjects" do
        ...
        before { create(:job_preferences_location, **location_preference, job_preferences: chemistry_job_preferences) }

        it "returns the jobseeker profiles with the key_stages specified in the filters" do
          expect(search.jobseeker_profiles).to contain_exactly(history_jobseeker_profile, chemistry_jobseeker_profile)
        end
      end
    end
    ...
    context "when multiple filters have been applied", :vcr do
      ...
      before { create(:job_preferences_location, **location_preference, job_preferences: job_preferences) }

      it "returns a jobseeker profile with an attribute matching any of the values used in the filter" do
        expect(search.jobseeker_profiles).to eq([jobseeker_profile])
      end
    end
  end
