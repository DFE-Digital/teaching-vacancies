...
The deployments to all environment share the same and simplified workflow. The deployment to review app (via Pull Request), staging, production, qa and research go through the CI/CD pipeline. Whilst the same sets of workflows can be used to deploy to `Dev`, a `git push` is required to commence deployment.
