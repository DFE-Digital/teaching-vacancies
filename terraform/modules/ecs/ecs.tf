...
resource "aws_ecs_task_definition" "migrate_vacancy_working_pattern_task" {
  family                   = "${var.ecs_service_web_task_name}_migrate_vacancy_working_pattern_task"
  container_definitions    = "${data.template_file.migrate_vacancy_working_pattern_container_definition.rendered}"
  requires_compatibilities = ["EC2"]
  network_mode             = "bridge"
  cpu                      = "256"
  memory                   = "512"
