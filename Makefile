...
production: ## production # Requires `CONFIRM_PRODUCTION` to be present (any value)
		$(shell if [[ $(CONFIRM_PRODUCTION) = "YES" ]]; then echo ""; else echo "Please enter "CONFIRM_PRODUCTION=YES" to run workflow"; fi)
		$(eval env=production)
...
terraform-app-destroy: terraform-app-init ## make qa destroy passcode=MyPasscode
	$(shell if [[ $(CONFIRM_DESTROY) = "YES" ]]; then echo ""; else echo "Please enter "CONFIRM_DESTROY=YES" to run workflow"; fi)
...
terraform-app-database-replace: terraform-app-init check-terraform-variables
	$(shell if [[ $(CONFIRM_REPLACE) = "YES" ]]; then echo ""; else echo "Please enter "CONFIRM_REPLACE=YES" to run workflow"; fi)
