...
#		$(if $(CONFIRM_PRODUCTION), , $(error Can only run with CONFIRM_PRODUCTION))
		$(shell if [[ $(CONFIRM_PRODUCTION) = "YES" ]]; then echo ""; else echo "Please enter "YES" to run workflow"; fi)
...
#	$(if $(CONFIRM_DESTROY), , $(error Can only run with CONFIRM_DESTROY))
	$(shell if [[ $(CONFIRM_DESTROY) = "YES" ]]; then echo ""; else echo "Please enter "YES" to run workflow"; fi)
