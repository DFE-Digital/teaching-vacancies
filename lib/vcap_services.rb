...
class VcapServices
  ...
  def service_url(service_type)
    ...
    raise "VCAP_SERVICES has no services of type '#{service_type}'" if candidates.empty?

    candidates.first.dig("credentials", "uri")
  end

  def named_service_url(service_type, name)
    candidates = services_of_type(service_type).select { |service| service["name"].include?(name) }
    raise "VCAP_SERVICES has no '#{name}' services of type '#{service_type}'" if candidates.empty?
