...
namespace :ons do
  ...
  task validate_polygons: :environment do
    LocationPolygon.find_each do |lp|
      puts lp.name
      puts "  -> #{lp.area.class}"
      orgs = Organisation.within_polygon(lp)
      puts "  -> #{orgs.count} orgs in #{lp.name}"
      puts "  -> #{orgs.pluck(:town).uniq.first(5).join(', ')}..."
    end
  end
end
